...
Version 0.4 calculates level, hit dice, BAB, base saves, class HP, and skill-rank allowances from the level-by-level Gestalt array. At every Gestalt level, BAB uses the higher progression category (full, then 3/4, then 1/2) and each base save uses good over poor, then applies that chosen class level's gain. This remains level-by-level when either track changes classes. Custom hit-die formulas are evaluated at every stored class level, including levels that grant zero or multiple hit dice. Automatic HP follows PF1e's health configuration, including its rounding, maximized-hit-die settings, and class-item sort order. When class HP is entered manually, PF1e stores only one aggregate value on the class item, so the module distributes that value over the class's hit dice before comparing each gestalt pair. Skill ranks use the better class allowance at each level and include Intelligence, minimum ranks, and background skills. Recorded favored-class HP and skill bonuses from both classes are applied additively after selecting the better class result. PF1e's **From Sources** tooltips consolidate class-derived HP, BAB, base saves, and skill-rank allowances into an accurate **Gestalt Classes** entry while preserving unrelated bonuses and background-skill transfers.
